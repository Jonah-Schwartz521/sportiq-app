# apps/api/tests/test_events_get_by_id.py
...
def _seed_event(event_id: int = TEST_ID):
    """Insert a known event row and commit, replacing any leftover."""
...
def test_get_event_by_id(client):
    ...
    # (Optional) sanity: ensure the row exists right now
    # with psycopg.connect(POSTGRES_DSN) as conn, conn.cursor() as cur:
    #     cur.execute("SELECT 1 FROM core.events WHERE event_id = %s;", (TEST_ID,))
